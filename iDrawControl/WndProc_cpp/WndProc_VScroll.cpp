...
LRESULT CALLBACK WndProc_VScroll(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, LPOBJSTRUCT pData);


LPOBJSTRUCT _subclass_MDIClient_VScroll(HWND hWnd, LPOBJSTRUCT pMDIClientChild1Data, bool reMoveBorder)
{
    LPOBJSTRUCT data = _subclass(hWnd, WndProc_VScroll, true);
    if (!data)return 0;
    data->draw.isNcCalcSize = true;
    data->draw.isNoNcPaint = true;
    data->param2 = pMDIClientChild1Data->param2;
    data->draw.offsetRect.left += 18;
    ...
    if (!data->drawScroll)
    {
        ...
        if (pCodeWnd)
        {
            ...
            IDrawScroll::SetBindControl(data->hWnd, true, true);
            ((IDrawScroll*)data->drawScroll)->m_hWndVBind = data->hWnd;
        }
    }
