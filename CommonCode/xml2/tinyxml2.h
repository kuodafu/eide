...
public:
    ...
    inline bool SaveFile(const_pointer filename = (const_pointer)L"", bool compact = false)
    {
...
public:
    ...
    inline int GetChildNodeCount(const_pointer pszPath = (const_pointer)L"") const
    {
        if (!m_doc)return 0;
    ...
    {
        if (!node)
            return (const_pointer)L"";
        return ToReturn(node->Value());
    }
    // 取节点名, 取得参数节点的名字, <节点名 属性名="属性值"/>
    // pszPath = 路径, 根节点/子节点/叶子节点, 或者 @1/@1/@2, 为0则为根节点
    inline const_pointer GetNodeName(const_pointer pszPath = (const_pointer)L"") const
    {
        if (!m_doc)return 0;
        tinyxml2::XMLNode* node;
        if (!GetNode(pszPath, node))
            return (const_pointer)L"";
        return ToReturn(node->Value());
    }
    ...
    // 取节点值, 取得参数节点的值, <节点名 属性名="属性值"/>
    // pszPath = 路径, 根节点/子节点/叶子节点, 或者 @1/@1/@2, 为0则为根节点
    inline const_pointer GetNodeValue(const_pointer pszPath = (const_pointer)L"") const
    {
        if (!m_doc)return 0;
        tinyxml2::XMLNode* node;
        if (!GetNode(pszPath, node))
    ...
    // isAttribute = 要获取的这个属性是否存在, 可以为0, 如果属性不存在, 这个值为0, 存在则为1
    inline const_pointer GetAttrValue(tinyxml2::XMLNode* node, const_pointer pszAttrName, const_pointer lpszDefValue = (const_pointer)L"", BOOL* isAttribute = 0) const
    {
        if (isAttribute)*isAttribute = FALSE;
        if (!node || !pszAttrName)
            return (const_pointer)L"";
    ...
    // lpszDefValue = 获取失败返回的默认值
    // isAttribute = 要获取的这个属性是否存在, 可以为0, 如果属性不存在, 这个值为0, 存在则为1
    inline const_pointer GetAttrValue(const_pointer pszPath, const_pointer pszAttrName, const_pointer lpszDefValue= (const_pointer)L"", BOOL* isAttribute = 0) const
    {
        if (isAttribute)*isAttribute = FALSE;
        tinyxml2::XMLNode* node;
        if (!GetNode(pszPath, node))
    ...
    // lpszDefValue = 获取失败返回的默认值
    // isAttribute = 要获取的这个属性是否存在, 可以为0, 如果属性不存在, 这个值为0, 存在则为1
    inline const_pointer GetAttrValue(int index, const_pointer pszAttrName, const_pointer lpszDefValue = (const_pointer)L"", BOOL* isAttribute = 0) const
    {
        if (isAttribute)*isAttribute = FALSE;
        tinyxml2::XMLNode* node;
        if (!GetNode(to_index(index + 1), node))
            return (const_pointer)L"";
        return GetAttrValue(node, pszAttrName, lpszDefValue, isAttribute);
    }
    //// 插入备注文本, 值为0则删除该备注文本
    ...
    inline bool InsertChildNode(const_pointer pszParentPath, const_pointer pszNodeName, const_pointer pszNodeValue = (const_pointer)L"", bool isCDATA = false, tinyxml2::XMLNode** ppNode = 0)
    {
    ...
    // pszNodeValue = 节点值  <节点名>节点值</节点名>
    // isCDATA = 如果本参数为真，则节点值文本将被 "<![CDATA[" 和 "]]>" 包围，这意味着该文本可以包含任意字符(以不影响CDATA解析为限}
    inline bool InsertChildNode(tinyxml2::XMLNode* parentNode, const_pointer pszNodeName, const_pointer pszNodeValue = (const_pointer)L"", bool isCDATA = false, tinyxml2::XMLNode** ppNode = 0)
    {
    ...
    // pszNodeValue = 节点值  <节点名>节点值</节点名>
    // isCDATA = 如果本参数为真，则节点值文本将被 "<![CDATA[" 和 "]]>" 包围，这意味着该文本可以包含任意字符(以不影响CDATA解析为限}
    inline bool InsertNode(const_pointer pszPath, const_pointer pszNodeName, const_pointer pszNodeValue = (const_pointer)L"", bool isCDATA = false, tinyxml2::XMLNode** ppNode = 0)
    {
        if (!m_doc || !pszNodeName)return 0;
        tinyxml2::XMLNode* node;
        if (!GetNode(pszPath, node)) return 0;
