...
enum SEARCH_MODE
{
    ...
    SEARCH_MODE_EDIT        = 0x0004,   // 下面使用编辑框而不是组合框
    SEARCH_MODE_NOEDIT      = 0x0005,   // 下面的组合框不能编辑, 只能选择
};
...
private:
    ...
    typedef struct _SEARCH_STRUCT
    {
        ...
        std::wstring    pszSearchLabel; // 搜索框左边的标签标题
...
public:
    ...
    inline LPCWSTR Show(HWND hWndParent = 0)
    {
        ...
        UINT dpi = getdpi(hWndParent);

        const int _24 = scale(24, dpi), _450 = scale(450, dpi), _500 = scale(500, dpi);
        const int _20 = scale(20, dpi), _460 = scale(460, dpi), _29 = scale(29, dpi);
        const int _10 = scale(10, dpi), _498 = scale(498, dpi), _126 = scale(126, dpi);
        int height = _24;
        m_data.nRadioHeight = 0;
        int mode = GetSearchMode();
        if (mode == SEARCH_MODE_NORMAL || mode == SEARCH_MODE_CHECKBOX
            || mode == SEARCH_MODE_EDIT || mode == SEARCH_MODE_NOEDIT)
        {
            RECT rcText = { 0, 0, _450, _500 };
            CalcTextWidth(rcText, psTips, dpi);
            height = rcText.bottom - rcText.top;
            if (height < _24) height = _24;
            else if (height > _500) height = _500;
        }

        if (mode == SEARCH_MODE_CHECKBOX)
        {
            const int box_width = _20;   // 文本左边框的宽度
            const int maxWidth = _460;   // 超过这个值就需要换行
            int nWidth = 0, nHeight = _29;
            for (RADIOBOX_STRUCT& item : m_radio)
            {
                // 框的宽度 + 文本宽度 + 10像素间隔

                RECT rcText = { 0, 0, _500, _24 };
                CalcTextWidth(rcText, item.text.c_str(), dpi);
                int width_text = rcText.right - rcText.left;

                item.nWidth = box_width + width_text;
                const int nowWidth = (item.nWidth + _10);
                nWidth += nowWidth;

                if (nWidth > maxWidth)
                {
                    // 换行了
                    nHeight += _29;   // 高度是24, 行间隔是5
                    nWidth = 0;
                    item.isWrap = TRUE;
                }
            }
        ...
        create(hWndParent, _498, _126 + height, style, dwExtendedStyle, pszCaption);
        if (!m_data.isOk)
            return 0;
...
public:
    ...
    // 置搜索左边的标签文本
    inline bool SetSearchLabel(LPCWSTR text)
    {
        m_data.pszSearchLabel = text;
        return true;
    }
    // 置取消按钮文本
    ...
    // 设置输入框的默认文本
    // pText = 文本
    inline bool SetDefaultText(LPCWSTR pText)
    {
        m_arr_text.clear();
...
public:
    ...
    // 设置搜索框输入的文本, 返回设置后的文本指针
    inline LPCWSTR SetResult(LPCWSTR pszResult)
    {
        if (!pszResult)
            pszResult = L"";
        m_data.pResult = pszResult;
        return !m_data.pResult.empty() ? m_data.pResult.c_str() : L"";
    }
    ...
    // 获取搜索按钮文本, 如果没有则返回空文本, 不会返回0
    inline LPCWSTR GetSearchLabel() const
    {
        return !m_data.pszSearchLabel.empty() ? m_data.pszSearchLabel.c_str() : L"搜索";
    }
...
private:
    inline INT_PTR InitDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
    {
        ...
        UINT dpi = GetDpiForWindow_d(hDlg);
#define _num(_s)  _##_s = scale(_s, dpi)
        const int 
            _num(11), _num(460), _num(140), _num(90), _num(200), _num(16),
            _num(500), _num(5), _num(40), _num(24), _num(10), _num(60), _num(31);
#undef _num
        const int margin = _11;  // 边距
        const int editWidth = _460;
        const int cbkWidth = _140;
        const int btnWidth = _90;
        const int height_cbb = _200;
        const int dialogWidth = editWidth + margin * 2 + _16;
        ...
        {
        case SEARCH_MODE_NOEDIT:    // 下面的组合框不能编辑, 只能选择
        case SEARCH_MODE_EDIT:      // 下面使用编辑框而不是组合框
        ...
        case SEARCH_MODE_NORMAL:    // 普通搜索, 一个标签, 一个可输入组合框, 两个按钮
        {
            ...
            if (m_clr)
                SetWindowTheme(hChild, m_clr->isDarkMode ? L"DarkMode_Explorer" : L"Explorer", 0);

            top += m_data.nTipsHeight + _5;
            pfn_subclass(hChild);
            int cbbHeight = _24;
            if (pData->nSearchMode == SEARCH_MODE_EDIT)
            {
                // 创建编辑框
                style = WS_TABSTOP | WS_VISIBLE | WS_GROUP | WS_CHILD | ES_AUTOHSCROLL | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_BORDER;
                styleEx = WS_EX_NOPARENTNOTIFY;
                hCbb = CreateControl(L"Edit", 0, styleEx, style, margin + _40, top, editWidth - _40, cbbHeight, hDlg, IDC_CBB_INPUT, 0);
                CDrawComboBox::InitEdit(hCbb, m_clr);
            }
            else
            {
                if(SEARCH_MODE_NOEDIT == pData->nSearchMode)
                    style = WS_TABSTOP | WS_VISIBLE | WS_CHILD | CBS_DROPDOWNLIST | CBS_HASSTRINGS;
                else
                    style = WS_TABSTOP | WS_VISIBLE | WS_CHILD | CBS_DROPDOWN | CBS_AUTOHSCROLL | CBS_HASSTRINGS;

                hCbb = CreateControl(L"ComboBox", 0, styleEx, style, margin + _40, top, editWidth - _40, height_cbb, hDlg, IDC_CBB_INPUT, 0);
                ...
                cbbHeight = rc.bottom - rc.top;
            }
            hCbbFocus = hCbb;
            CreateControl(L"static", L"搜索", styleEx, styleStatic, margin, top, _40, cbbHeight, hDlg, 0, 0);
            top += cbbHeight + _5;

            if (pData->nSearchMode == SEARCH_MODE_CHECKBOX)
            {
                int left = margin;
                const int radio_height = _24;
                bool isFirst = true;
                for (RADIOBOX_STRUCT& item : m_radio)
                {
        ...
        {
            hCbb = CreateControl(L"ComboBox", 0, styleEx, style, margin + _40, top, editWidth - _40, height_cbb, hDlg, IDC_CBB_MODE);
            //CDrawComboBox::InitCombobox(hCbb, m_clr);
            GetWindowRect(hCbb, &rc);
            const int cbbHeight = rc.bottom - rc.top;
            CreateControl(L"static", L"类型", styleEx, styleStatic, margin, top, _40, cbbHeight, hDlg, 0);
            top += cbbHeight + _5;
            ...
            style = WS_TABSTOP | WS_VISIBLE | WS_CHILD | CBS_DROPDOWN | CBS_AUTOHSCROLL | CBS_HASSTRINGS;
            CreateControl(L"static", L"搜索", styleEx, styleStatic, margin, top, _40, cbbHeight, hDlg, 0);
            hCbb = CreateControl(L"ComboBox", 0, styleEx, style, margin + _40, top, editWidth - _40, height_cbb, hDlg, IDC_CBB_INPUT);
            //CDrawComboBox::InitCombobox(hCbb, m_clr);
            top += cbbHeight + _5;
            hCbbFocus = hCbb;
            break;
        ...
        if (pData->nSearchMode == SEARCH_MODE_EDIT)
        {
            if (m_arr_text.size() > 0)
            {
                std::wstring& text = m_arr_text.front();
                SetWindowTextW(hCbb, text.c_str());
            }
        }
        else
        {
        ...
        int left = (rc.right - rc.left) / 2;
        HWND hChild = CreateControl(L"button", pszOk, styleEx, style, left - btnWidth - _5, top, btnWidth, _31, hDlg, IDOK);
        if (m_clr)
            SetWindowTheme(hChild, m_clr->isDarkMode ? L"DarkMode_Explorer" : L"Explorer", 0);
        ...
        hChild = CreateControl(L"button", pszCancel, styleEx, style, left + _5, top, btnWidth, _31, hDlg, IDCANCEL);
        if (m_clr)
            SetWindowTheme(hChild, m_clr->isDarkMode ? L"DarkMode_Explorer" : L"Explorer", 0);
        pfn_subclass(hChild);
    ...
    static bool _key_down(HWND hWnd, int key)
    {
        ...
        for (RADIOBOX_STRUCT& item : arr)
        {
            ...
            for (int i = 1; i < 4; i++)
            {
                ...
                if (pKey[i])
                    isCheck = isCheck && ((GetAsyncKeyState(pKey[i]) & 0x8000) != 0);
            }
