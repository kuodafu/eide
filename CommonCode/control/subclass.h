...
// 提供一个模板类型
template<typename T>
struct tagSUBCLASSSTRUCT_YEMPLATE : private __subclass::tagMAKEPROCDATA
{
    // 私有继承
    HWND hWnd;              // 子类化的窗口句柄
    WNDPROC oldProc;        // 旧窗口过程
...
typedef LRESULT(__stdcall* SUBWNDPROC)(SUBCLASSSTRUCT*, UINT, WPARAM, LPARAM);


// 返回值强转成 WNDPROC 就可以在注册类时赋值, 窗口过程的第一个参数就是这个返回值
// proc = 子类化回调函数
// param = 用户数据
inline SUBCLASSSTRUCT* MakeProc(SUBWNDPROC proc, void* param)
{
    ...
    SUBCLASSSTRUCT* ptr = (SUBCLASSSTRUCT*)pData;
    ptr->param = param;
...
// param = 用户数据
template<typename T>
inline SUBCLASSSTRUCT* MakeProc(T* pThis, LRESULT(__stdcall T::* fnCallback)(SUBCLASSSTRUCT*, UINT, WPARAM, LPARAM), void* param)
{
...
inline bool __stdcall SubClassWindow(HWND hWnd, SUBWNDPROC proc, void* param)
{
    ...
    SUBCLASSSTRUCT* ptr = MakeProc(proc, param);
    if (!ptr)return false;
    ptr->hWnd = hWnd;
...
// param = 用户数据
template<typename T> 
inline bool __stdcall SubClassWindow( HWND hWnd, T* pThis, LRESULT(__stdcall T::* proc)(SUBCLASSSTRUCT*, UINT, WPARAM, LPARAM), void* param)
{
    if (!IsWindow(hWnd)) return false;
    SUBCLASSSTRUCT* ptr = MakeProc(pThis, proc, param);
    if (!ptr)return false;
    ptr->hWnd = hWnd;
    ptr->oldProc = (WNDPROC)SetWindowLongPtrW(hWnd, GWLP_WNDPROC, (LONG_PTR)ptr);
    // 返回的这个回调函数可以用作子类化的回调函数
