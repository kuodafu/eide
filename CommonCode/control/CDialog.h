#pragma once
#include <control/CDrawComboBox.h>
#include <vector>
#include <string>
...
protected:
    ...
    inline int scale(int x, UINT dpi) const
    {
        return MulDiv(x, dpi, USER_DEFAULT_SCREEN_DPI);
    }
    inline UINT getdpi(HWND hWnd) const
    {
        if(IsWindow(hWnd))
            return GetDpiForWindow_d(hWnd);
...
public:
    CDialog() : m_hWnd(0), m_clr(0), m_arg{ 0 }
    {
...
protected:
    ...
    inline virtual INT_PTR OnCtlColor(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
    {
        ...
        HDC hdc = (HDC)wParam;
        THEMECOLOR_STRUCT& clr = *m_clr;

        HBRUSH hbrBack = CThemeColor::GetBrush(clr.crBack);
        ...
        {
            hbrBack = CThemeColor::GetBrush(clr.crBan);
            crText = clr.crTextBan;
            crBack = clr.crBan;
        }
        else
        {
            hbrBack = CThemeColor::GetBrush(clr.crBack);
            crText = clr.crText;
            crBack = clr.crBack;
        }
    ...
    inline virtual INT_PTR OnThemeColorChanged(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
    {
        if (m_clr)
        {
            PTHEMECOLOR_STRUCT pClr = (PTHEMECOLOR_STRUCT)lParam;
            memcpy(m_clr, pClr, sizeof(THEMECOLOR_STRUCT));
            CThemeColor::UpdateAllWindow(hDlg, true);
        }
        return true;
    }
...
protected:
    ...
    static HFONT DefautlFont(UINT dpi)
    {
        static UNMAP<UINT, HFONT> map;
        HFONT& hFont = map[dpi];
        if (!hFont)
        {
            LOGFONTW lf = { 0 };
    ...
    static int CalcTextWidth(RECT& rcText, LPCWSTR pszText, UINT dpi)
    {
        if (!pszText || !*pszText)
            return 0;
        static HDC hdc;
        ...
        auto hOldFont = SelectObject(hdc, DefautlFont(dpi));

        int ret = DrawTextW(hdc, pszText, -1, &rcText, DT_CALCRECT | DT_NOPREFIX | DT_WORDBREAK);
                              ...
                              HWND hParent, INT_PTR id = 0, LPCWSTR lpszTips = L"")
    {
        ...
        SendMessageW(hWnd, WM_SETFONT, (WPARAM)DefautlFont(dpi), 0);
        return hWnd;
    }
    ...
    {
        return CThemeColor::GetThemeColorFromMap(ret);
    }
    ...
    static INT_PTR OnNotify_Button(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam, PTHEMECOLOR_STRUCT pClr)
    {
            ...
            //case CDDS_PREERASE:
        {
            ...
            const int left_offset = MulDiv(16, dpi, USER_DEFAULT_SCREEN_DPI);
