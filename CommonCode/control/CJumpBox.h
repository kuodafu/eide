...
public:
    ...
    inline LPCWSTR Show(HWND hWndParent = 0)
    {
        ...
        UINT dpi = GetDpiForWindow_d(hWndParent);
        if (!dpi)
            dpi = GetDpiForWindow_d(GetDesktopWindow());

        const int _498 = scale(498, dpi), _218 = scale(218, dpi);
        LPCWSTR pszCaption = GetCaption();
        m_data.isOk = FALSE;
        m_data.hParent = hWndParent;
        popup(hWndParent, _498, _218, style, dwExtendedStyle, pszCaption);
        if (!m_data.isOk)
            return 0;
...
#define _num(_s)  _##_s = scale(_s, dpi)
        const int
            _num(11), _num(460), _num(140), _num(90), _num(200), _num(70),
            _num(120), _num(5), _num(80), _num(24), _num(20), _num(60), _num(55);
#undef _num

        const int margin = _11;  // 边距
        const int editWidth = _460;
        const int cbkWidth = _140;
        const int btnWidth = _140;
        const int height_cbb = _200;
        const int width_cbb = editWidth - _55;
        const int static_height = _20;
        const int right = margin + _55 + width_cbb;
        ...
        pData->hCbb = CreateControl(L"ComboBox", 0, styleEx, style, margin + _55, top, width_cbb, height_cbb, hDlg, IDC_CBB_INPUT);
        CDrawComboBox::InitCombobox(pData->hCbb, m_clr);
        RECT rc;
        GetWindowRect(pData->hCbb, &rc);
        const int cbbHeight = rc.bottom - rc.top;
        CreateControl(L"static", L"跳转位置", styleEx, styleStatic, margin, top, _55, cbbHeight, hDlg, 0);
        top += cbbHeight + 5;
        ...
        HWND hChild = 0;
        top = saveTop - _5 - (static_height / 2);
        const int btnHeight = static_height * 2 + 5 + (static_height / 2);
        left = right - btnWidth;
        hChild = CreateControl(L"button", L"跳转", styleEx, styleBtn | BS_DEFPUSHBUTTON, left, top, btnWidth, btnHeight, hDlg, IDOK);
        if (m_clr)
            SetWindowTheme(hChild, m_clr->isDarkMode ? L"DarkMode_Explorer" : L"Explorer", 0);

        top += btnHeight + 5;
        hChild = CreateControl(L"button", L"取消", styleEx, styleBtn, left, top, btnWidth, btnHeight, hDlg, IDCANCEL);
        if (m_clr)
            SetWindowTheme(hChild, m_clr->isDarkMode ? L"DarkMode_Explorer" : L"Explorer", 0);
        ...
        top += static_height + _5;
        top += static_height + _5;
        pData->h10 = CreateControl(L"button", L"10进制", styleEx, style | WS_GROUP, left, top, _70, static_height, hDlg, IDC_RADIO_10);
        top += static_height + _5;
        pData->h16 = CreateControl(L"button", L"16进制", styleEx, style, left, top, _70, static_height, hDlg, IDC_RADIO_16);
